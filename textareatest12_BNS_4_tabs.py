...
app.layout = html.Div([
    ...
    html.Div([
        dcc.Tabs(id='tabs-example', value='tab-1', children=[
            ...
            dcc.Tab(label='Block v Screen', value='tab-4')
        ]),
...
def render_content(n_clicks, value, tab):
    ...
    if n_clicks > 0:
        ...
        try:
            ...
            # *Summation breakdown by product Screen(Trade) vs Block
            df_product_summary_origin = df_blocks.groupby(['Type', 'CC'])['Qty'].sum().reset_index()
            origin_table = pd.pivot_table(df_product_summary_origin, values='Qty', index=['Type'],
                    columns=['CC'], aggfunc=np.sum, fill_value=0).reset_index()
            
            origin_records = origin_table.to_dict('records')
            origin_columns = [{"name": i, "id": i, } for i in (origin_table.columns)]
    ...
    if tab == 'tab-1':
        ...
        return html.Div([
            html.H3('Block v Screen'),
            dash_table.DataTable(  # *g4 strip Table
                data=origin_records, columns=origin_columns, page_size=15, sort_action='native', filter_action='native')
        ])    
    else:
        print('else...')
        return html.Div([
